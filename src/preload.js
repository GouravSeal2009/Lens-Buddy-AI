...
try {
  contextBridge.exposeInMainWorld('electronAPI', {
    ...
    takeStealthScreenshot: (options = {}) => {
      console.log('PreloadAPI: takeStealthScreenshot called');
      return ipcRenderer.invoke('take-stealth-screenshot', options).catch(err => {
    ...
    setAiTier: (tier) => {
      console.log('PreloadAPI: setAiTier called:', tier);
      return ipcRenderer.invoke('set-ai-tier', tier).catch(err => {
        console.error('PreloadAPI: setAiTier error:', err);
        return { error: err.message };
      });
    },

    getAiTier: () => {
      console.log('PreloadAPI: getAiTier called');
      return ipcRenderer.invoke('get-ai-tier').catch(err => {
        console.error('PreloadAPI: getAiTier error:', err);
        return 'fast';
      });
    },

    getAiRoutingPlan: () => {
      console.log('PreloadAPI: getAiRoutingPlan called');
      return ipcRenderer.invoke('get-ai-routing-plan').catch(err => {
        console.error('PreloadAPI: getAiRoutingPlan error:', err);
        return [];
      });
    },

    getAiProviderStatus: () => {
      console.log('PreloadAPI: getAiProviderStatus called');
      return ipcRenderer.invoke('get-ai-provider-status').catch(err => {
        console.error('PreloadAPI: getAiProviderStatus error:', err);
        return { tier: 'fast', activeProviderIndex: 0, activeModelName: null, chain: [] };
      });
    },
    ...
    answerVoiceInput: (payload) => {
      console.log('PreloadAPI: answerVoiceInput called');
      return ipcRenderer.invoke('answer-voice-input', payload).catch(err => {
        console.error('PreloadAPI: answerVoiceInput error:', err);
        return { error: err.message };
      });
    },
    ...
    setClickThrough: (enabled) => {
      console.log('PreloadAPI: setClickThrough called:', enabled);
      return ipcRenderer.invoke('set-click-through', enabled).catch(err => {
        console.error('PreloadAPI: setClickThrough error:', err);
        return { error: err.message };
      });
    },
    ...
    resizeWindow: (width, height) => {
      console.log('PreloadAPI: resizeWindow called:', width, height);
      return ipcRenderer.invoke('resize-window', width, height).catch(err => {
        console.error('PreloadAPI: resizeWindow error:', err);
        return { error: err.message };
      });
    },

    resizeWindowLive: (width, height) => {
      ipcRenderer.send('resize-window-live', width, height);
    },

    moveWindowLive: (x, y) => {
      ipcRenderer.send('move-window-live', x, y);
    },

    startWindowDrag: () => {
      return ipcRenderer.invoke('start-window-drag').catch(err => {
        console.error('PreloadAPI: startWindowDrag error:', err);
        return { error: err.message };
      });
    },

    stopWindowDrag: () => {
      return ipcRenderer.invoke('stop-window-drag').catch(err => {
        console.error('PreloadAPI: stopWindowDrag error:', err);
        return { error: err.message };
      });
    },

    startWindowResize: () => {
      return ipcRenderer.invoke('start-window-resize').catch(err => {
        console.error('PreloadAPI: startWindowResize error:', err);
        return { error: err.message };
      });
    },

    updateWindowResize: (cursorX, cursorY) => {
      return ipcRenderer.invoke('update-window-resize', cursorX, cursorY).catch(err => {
        console.error('PreloadAPI: updateWindowResize error:', err);
        return { error: err.message };
      });
    },

    stopWindowResize: () => {
      return ipcRenderer.invoke('stop-window-resize').catch(err => {
        console.error('PreloadAPI: stopWindowResize error:', err);
        return { error: err.message };
      });
    },

    toggleWindowVisibility: () => {
      console.log('PreloadAPI: toggleWindowVisibility called');
      return ipcRenderer.invoke('toggle-window-visibility').catch(err => {
        console.error('PreloadAPI: toggleWindowVisibility error:', err);
        return { error: err.message };
      });
    },
    ...
    onStatusMessage: (callback) => {
      const handler = (event, message) => {
        console.log('PreloadAPI: onStatusMessage event received, message:', message);
        try {
          callback(message);
        } catch (err) {
          console.error('PreloadAPI: onStatusMessage callback error:', err);
        }
      };
      ipcRenderer.on('status-message', handler);
      return () => {
        console.log('PreloadAPI: removing onStatusMessage listener');
        ipcRenderer.removeListener('status-message', handler);
      };
    },

    onClickThroughChanged: (callback) => {
      const handler = (event, enabled) => {
        console.log('PreloadAPI: onClickThroughChanged event received, enabled:', enabled);
        try {
          callback(enabled);
        } catch (err) {
          console.error('PreloadAPI: onClickThroughChanged callback error:', err);
        }
      };
      ipcRenderer.on('click-through-changed', handler);
      return () => {
        console.log('PreloadAPI: removing onClickThroughChanged listener');
        ipcRenderer.removeListener('click-through-changed', handler);
      };
    },

    onAiRoutingWarning: (callback) => {
      const handler = (event, warning) => {
        console.log('PreloadAPI: onAiRoutingWarning event received:', warning?.type);
        try {
          callback(warning);
        } catch (err) {
          console.error('PreloadAPI: onAiRoutingWarning callback error:', err);
        }
      };
      ipcRenderer.on('ai-routing-warning', handler);
      return () => {
        console.log('PreloadAPI: removing onAiRoutingWarning listener');
        ipcRenderer.removeListener('ai-routing-warning', handler);
      };
    },

    onAiProviderStatus: (callback) => {
      const handler = (event, status) => {
        console.log('PreloadAPI: onAiProviderStatus event received');
        try {
          callback(status);
        } catch (err) {
          console.error('PreloadAPI: onAiProviderStatus callback error:', err);
        }
      };
      ipcRenderer.on('ai-provider-status', handler);
      return () => {
        console.log('PreloadAPI: removing onAiProviderStatus listener');
        ipcRenderer.removeListener('ai-provider-status', handler);
      };
    },
