...
class VoskLiveTranscriber:
    def __init__(self):
        """Initialize Vosk with a compact Indian English model for better accent fit"""

        # Prefer Indian English for this app's target users while keeping downloads manageable.
        self.model_name = "vosk-model-small-en-in-0.4"
        self.model_url = "https://alphacephei.com/vosk/models/vosk-model-small-en-in-0.4.zip"
        ...
        if not self.model_path.exists():
            self.send_status("downloading", "Downloading Indian English voice model (one-time, ~36MB)...")
            self.download_model()

        self.send_status("loading", "Loading voice model...")
